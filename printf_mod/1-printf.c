...
int _printf(const char *format, ...)
{
	...
	while (format[countformat] != '\0')
	{
		...
		{ write(1, "%", 1); printed_count++; countformat += 2; continue;}
		...
		{
			counting_list = 0; matched = 0;
			while (ops[counting_list].op != NULL)
			{
				if (format[countformat + 1] == ops[counting_list].op[0])
				{ printed_count += ops[counting_list].print(args); countformat += 2; matched = 1; break;}
				counting_list++;
			}
			if (matched == 0)
	...
	va_end(args); return (printed_count);
}
